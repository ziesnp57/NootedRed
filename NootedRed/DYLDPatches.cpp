...
void DYLDPatches::wrapCsValidatePage(vnode *vp, memory_object_t pager, memory_object_offset_t page_offset,
	...
	if (getKernelVersion() >= KernelVersion::Sonoma) {
		...
		const DYLDPatch patchOpenGL = {kCatalinaAddrLibGetBaseArrayModeReturnOriginal, kCatalinaAddrLibGetBaseArrayModeReturnPatched, "Catalina OPENGL changed"};
		patchOpenGL.apply(const_cast<void *>(data), PAGE_SIZE);
	} else {
		// 未知版本
		return;
    ...
    if (getKernelVersion() >= KernelVersion::Sonoma) {
        const DYLDPatch patches[] = {
            ...
            {kVAFactoryCreateGraphicsEngineSonomaOriginal,
                ...
                kVAFactoryCreateGraphicsEngineSonomaPatched,
                kVAFactoryCreateGraphicsEngineSonomaPatchedMask,
                "VAFactory::createGraphicsEngine"},
			{kVAFactoryCreateImageBltVenturaOriginal, kVAFactoryCreateImageBltVenturaOriginalMask, kVAFactoryCreateImageBltVenturaPatched,
				kVAFactoryCreateImageBltVenturaPatchedMask, "VAFactory::createImageBlt"},
            {kVAFactoryCreateVPSonomaOriginal, kVAFactoryCreateVPSonomaOriginalMask, kVAFactoryCreateVPSonomaPatched,
